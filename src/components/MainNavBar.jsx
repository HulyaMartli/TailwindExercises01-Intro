...
export default function MainNavBar() {
    ...
    return (
        <header className="bg-black xs:py-0 py-6 lg:py-12 text-mov-grey uppercase">
            <div className="container flex items-center justify-between space-x-8 lg:space-x-16 xs:h-20">
                ...
                <div className="block md:hidden xs:right-0 xs:fixed">
                    <div className="space-y-1 cursor-pointer me-4" onClick={handleMenuClick}>
                        <div className="bg-mov-grey w-8 h-1 rounded-full"></div>
                        <div className="bg-mov-grey w-8 h-1 rounded-full"></div>
                        <div className="bg-mov-grey w-8 h-1 rounded"></div>
                    </div>
                </div>

                {/* NAVIGATION */}
                <nav className={`${showMenu} md:flex justify-between lg:flex-1 md:translate-y-0 md:bg-transparent md:pt-0 md:mt-0 xs:mt-24 xs:bg-black xs:h-screen xs:translate-y-80 xs:pt-5 xs:overflow-hidden z-50`} onClick={handleMenuClick}>
                    {/* MENU */}
